...
class asyncSession(object):
    def process_response(self, request, response):
        s = SessionStore(session_key=request.session.session_key)
        if 'openid' in s.keys():
            request.session['openid'] = s['openid']
        return response
...
def Transfers(request):
    ...
    price = request.POST.get('money', None)
    price = float(price) * 100
    price = int(price)  # 提现金额（分）
    sprice = float(price) / 100  # 提现金额（元）
    code = get_random_code()
    user = WX_User.objects.get(openid=openid)
    # 求可提余额
    if openid:
        couple_list = SpreadCouple.objects.filter(spread_user__username=openid)
        money = 0  # 粉丝消费总额
        for item in couple_list:
            mopenid = item.wx_user.openid
            all_order = Order.objects.filter(user__openid=mopenid, status=u'充值成功')
            for i in all_order:
                money += i.price
    commission = int(money * 0.1)  # 分成金额

    if user.spread_first == True:
        user.spread_first = False
        user.save()
        spread_yue = SpreadYuE.objects.create(user=user)
        yue = commission
    else:
        spread_yue = SpreadYuE.objects.get(user=user)
        yue = commission - spread_yue.total_price  # 余额 = 分成金额 - 提现总额
    ...
    if yue >= sprice:
        ...
        data = {
            ...
            "mchid": MchId,  # 商户号
            "nonce_str": code,  # 字符
            "partner_trade_no": code,  # 商户提现单号
            "openid": openid,
            "check_name": "NO_CHECK",
            "amount": price,
            "desc": "提现",
            "spbill_create_ip": "101.200.203.82",
        }

        data["sign"] = make_sign(data)

        transfers = requests.post("https://api.mch.weixin.qq.com/mmpaymkttransfers/promotion/transfers",
                             data=make_xml(data).encode('utf-8'),
                             cert=("cert/apiclient_cert.pem", "cert/apiclient_key.pem"))

        print analysis_xml(transfers.content)
        wechat = analysis_xml(transfers.content)
        if analysis_xml(transfers.content)['result_code'] == 'SUCCESS':
            spread_yue.total_price += sprice  # 记录累积提现总额(元)
            spread_yue.save()
            banlance = commission - spread_yue.total_price
            # 操作记录
            trans = TransRecord.objects.create(user=user,
                                               info=u'推广员提款',
                                               price=sprice,
                                               code=code,
            ...
            vdata = {'errno': 500,
                     'msg': 'success'}
            return HttpResponse(json.dumps(vdata), content_type="application/json")
        else:
